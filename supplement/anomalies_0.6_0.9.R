...
Raw_Data <- dbGetQuery(mydb, "SELECT * FROM site JOIN entity USING (site_id) JOIN sample USING (entity_id) JOIN original_chronology USING (sample_id) JOIN d18O USING (sample_id)
                       WHERE (interp_age BETWEEN 500 AND 1000);")
Raw_Data <- Raw_Data %>% filter(entity_status != "superseded")

#write.csv(Raw_Data, "rawdat_05_10.csv", row.names = F)
...
for (i in unique(Raw_Data$entity_id)){ # for every entity
  subdat <- Raw_Data %>% filter(entity_id == i & interp_age >= 500 & interp_age <= 1000)
  if (nrow(subdat) <= 1){ next }
  mean_res <- get_ent_sampling(entity_id = i, age_start = 500, age_end = 1000)$sampling_mean
...
for (i in 1:length(res_out$entity_id)){
  ...
  subdat <- Raw_Data %>% filter(entity_id == ent_id & interp_age >=500 & interp_age <= 1000)
...
anom_2bp <- anom_2bp %>% filter(ttest_Pval <= 0.001) #24
...
subdat <- dtrend_dat %>% filter(entity_id == 178)
sub_bp <- bp_dat %>% filter(entity_id == 178)
...
sub_hsd <- TukeyHSD(sub.av)$grp
grps <- rbind(grps, data.frame(entity_id = rep(178,3), group = 2:4))
...
for (i in unique(grps$entity_id)){
  ...
  subdat <- dtrend_dat %>% filter(interp_age >= 500 & interp_age <= 1000 & 
                                    entity_id == i)
...
anom_bp <- anom_bp %>% filter(diff_from_base_sd >= 0)  ##34->16
...
## no regional coherence

# sites with signal verus sites with no signal
nrow(all_dat %>% group_by(site_id) %>% summarise(n())) #33
nrow(no_signal %>% group_by(site_id) %>% summarise(n())) #55

33/(33+55) #37.5% (below threshold)

## save dat
write.csv(all_dat, "anom_600_900.csv", row.names = F)
write.csv(all_dat, "nosignal_600_900.csv", row.names = F)
