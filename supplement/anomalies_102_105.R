## Constrain 10.2-10.5 kyr signals
...
Raw_Data <- dbGetQuery(mydb, "SELECT * FROM site JOIN entity USING (site_id) JOIN sample USING (entity_id) JOIN original_chronology USING (sample_id) JOIN d18O USING (sample_id)
                       WHERE (interp_age BETWEEN 10100 AND 10600);")
...
for (i in unique(Raw_Data$entity_id)){ # for every entity
  subdat <- Raw_Data %>% filter(entity_id == i & interp_age >= 10100 & interp_age <= 10600)
  if (nrow(subdat) <= 1){ next }
  mean_res <- get_ent_sampling(entity_id = i, age_start = 10100, age_end = 10600)$sampling_mean
...
for (i in 1:length(res_out$entity_id)){
  ...
  subdat <- Raw_Data %>% filter(entity_id == ent_id & interp_age >=10100 & interp_age <= 10600)
...
for (i in unique(grps$entity_id)){
  ...
  subdat <- dtrend_dat %>% filter(interp_age >= 10100 & interp_age <= 10600 & 
                                    entity_id == i)
...
nrow(all_dat %>% group_by(site_id) %>% summarise(n())) #8
nrow(no_signal %>% group_by(site_id) %>% summarise(n())) #25

8/(8+25) #25% (below threshold)

## save dat
write.csv(all_dat, "anom_10200_10500.csv", row.names = F)
write.csv(no_signal, "nosignal_10200_10500.csv", row.names = F)
