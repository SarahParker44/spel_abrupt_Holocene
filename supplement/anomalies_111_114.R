...
Raw_Data <- dbGetQuery(mydb, "SELECT * FROM site JOIN entity USING (site_id) JOIN sample USING (entity_id) JOIN original_chronology USING (sample_id) JOIN d18O USING (sample_id)
                       WHERE (interp_age BETWEEN 11000 AND 11500);")

Raw_Data <- Raw_Data %>% filter(entity_status != "superseded")

#write.csv(Raw_Data, "rawdat_110_115.csv", row.names = F)
...
for (i in unique(Raw_Data$entity_id)){ # for every entity
  subdat <- Raw_Data %>% filter(entity_id == i & interp_age >= 11000 & interp_age <= 11500)
  if (nrow(subdat) <= 1){ next }
  mean_res <- get_ent_sampling(entity_id = i, age_start = 11000, age_end = 11500)$sampling_mean
...
for (i in 1:length(res_out$entity_id)){
  ...
  subdat <- Raw_Data %>% filter(entity_id == ent_id & interp_age >=11000 & interp_age <= 11500)
...
for (i in unique(grps$entity_id)){
  ...
  subdat <- dtrend_dat %>% filter(interp_age >= 11000 & interp_age <= 11500 & 
                                    entity_id == i)
...
nrow(all_dat %>% group_by(site_id) %>% summarise(n())) #11
nrow(no_signal %>% group_by(site_id) %>% summarise(n())) #15

11/(11+15) #44% (above threshold)

## save dat
write.csv(all_dat, "anom_11100_11400.csv", row.names = F)
write.csv(no_signal, "nosignal_11100_11400.csv", row.names = F)
