...
Raw_Data <- dbGetQuery(mydb, "SELECT * FROM site JOIN entity USING (site_id) JOIN sample USING (entity_id) JOIN original_chronology USING (sample_id) JOIN d18O USING (sample_id)
                       WHERE (interp_age BETWEEN 3500 AND 4000);")
Raw_Data <- Raw_Data %>% filter(entity_status != "superseded")

#write.csv(Raw_Data, "rawdat_35_40.csv", row.names = F)
...
for (i in unique(Raw_Data$entity_id)){ # for every entity
  subdat <- Raw_Data %>% filter(entity_id == i & interp_age >= 3500 & interp_age <= 4000)
  if (nrow(subdat) <= 1){ next }
  mean_res <- get_ent_sampling(entity_id = i, age_start = 3500, age_end = 4000)$sampling_mean
...
for (i in 1:length(res_out$entity_id)){
  ...
  subdat <- Raw_Data %>% filter(entity_id == ent_id & interp_age >=3500 & interp_age <= 4000)
...
anom_2bp <- anom_2bp %>% filter(ttest_Pval <= 0.001) #22
...
for (i in unique(grps$entity_id)){
  ...
  subdat <- dtrend_dat %>% filter(interp_age >= 3500 & interp_age <= 4000 & 
                                    entity_id == i)
...
anom_bp <- anom_bp %>% filter(diff_from_base_sd >= 0)  ##18->8
...
nrow(all_dat %>% group_by(site_id) %>% summarise(n())) #25
nrow(no_signal %>% group_by(site_id) %>% summarise(n())) #33

25/(25+33) #43% (above threshold)

## save dat
write.csv(all_dat, "anom_3600_3900.csv", row.names = F)
write.csv(no_signal, "nosignal_3600_3900.csv", row.names = F)
