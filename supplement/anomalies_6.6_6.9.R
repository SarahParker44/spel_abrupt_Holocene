...
Raw_Data <- dbGetQuery(mydb, "SELECT * FROM site JOIN entity USING (site_id) JOIN sample USING (entity_id) JOIN original_chronology USING (sample_id) JOIN d18O USING (sample_id)
                       WHERE (interp_age BETWEEN 6500 AND 7000);")
Raw_Data <- Raw_Data %>% filter(entity_status != "superseded")

#write.csv(Raw_Data, "rawdat_65_70.csv", row.names = F)

## Select entities with a sufficient resolution (of 30 years)
min_res <- 30
...
for (i in unique(Raw_Data$entity_id)){ # for every entity
  subdat <- Raw_Data %>% filter(entity_id == i & interp_age >= 6500 & interp_age <= 7000)
  if (nrow(subdat) <= 1){ next }
  mean_res <- get_ent_sampling(entity_id = i, age_start = 6500, age_end = 7000)$sampling_mean
...
for (i in 1:length(res_out$entity_id)){
  ...
  subdat <- Raw_Data %>% filter(entity_id == ent_id & interp_age >=6500 & interp_age <= 7000)
...
anom_2bp <- anom_2bp %>% filter(ttest_Pval <= 0.001) #16->14
...
x3 <- x %>% filter(n_bp == 3) # 11

# visualise entities
#for (i in seq(9,28,9)){
i=9
entities <- x3$entity_id[(9-8):9]
subdat <- dtrend_dat %>% filter(entity_id %in% entities)
sub_bp <- bp_dat %>% filter(entity_id %in% entities)
ggplot() + geom_line(data = subdat, aes(x = interp_age, y = d18O_detrended)) +
  geom_vline(data = sub_bp, aes(xintercept = bp), col = "red") +
...
for (i in unique(grps$entity_id)){
  ...
  subdat <- dtrend_dat %>% filter(interp_age >= 6500 & interp_age <= 7000 & 
                                    entity_id == i)
...
anom_bp <- anom_bp %>% filter(diff_from_base_sd >= 0)  ##17->9
...
nrow(all_dat %>% group_by(site_id) %>% summarise(n())) #17
nrow(no_signal %>% group_by(site_id) %>% summarise(n())) #35

17/(17+35) #32% (above threshold)

## save dat
write.csv(all_dat, "anom_6600_6900.csv", row.names = F)
write.csv(no_signal, "nosignal_6600_6900.csv", row.names = F)
