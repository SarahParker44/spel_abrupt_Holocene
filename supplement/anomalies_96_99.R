...
Raw_Data <- dbGetQuery(mydb, "SELECT * FROM site JOIN entity USING (site_id) JOIN sample USING (entity_id) JOIN original_chronology USING (sample_id) JOIN d18O USING (sample_id)
                       WHERE (interp_age BETWEEN 9500 AND 10000);")
Raw_Data <- Raw_Data %>% filter(entity_status != "superseded")

#write.csv(Raw_Data, "rawdat_95_100.csv", row.names = F)
...
for (i in unique(Raw_Data$entity_id)){ # for every entity
  subdat <- Raw_Data %>% filter(entity_id == i & interp_age >= 9500 & interp_age <= 10000)
  if (nrow(subdat) <= 1){ next }
  mean_res <- get_ent_sampling(entity_id = i, age_start = 9500, age_end = 10000)$sampling_mean
...
for (i in 1:length(res_out$entity_id)){
  ...
  subdat <- Raw_Data %>% filter(entity_id == ent_id & interp_age >=9500 & interp_age <= 10000)
...
for (i in unique(grps$entity_id)){
  ...
  subdat <- dtrend_dat %>% filter(interp_age >= 9500 & interp_age <= 10000 & 
                                    entity_id == i)
...
nrow(all_dat %>% group_by(site_id) %>% summarise(n())) #10
nrow(no_signal %>% group_by(site_id) %>% summarise(n())) #29

10/(10+29) #25% (below threshold)

## save dat
write.csv(all_dat, "anom_9600_9900.csv", row.names = F)
write.csv(no_signal, "nosignal_9600_9900.csv", row.names = F)
